...
type InvoiceB2BPageProps = { userType: 'admin' | 'retailer' };

export default function InvoiceB2BPage({ userType }: InvoiceB2BPageProps) {
  const [invoiceData, setInvoiceData] = useState<{
    ...
    products: [
      { "Product Name": "", "SKU": "", "Brand": "", "Sku Code": "", "Product Description": "", "Qty": 1, "Rate (₹)": 0, "GST %": 0, "GST Amt (₹)": 0, "Transport Cost (₹)": 0, "Total (₹)": 0 }
    ],
    summaryRow: [],
    taxSummary: {},
    grandTotal: "",
...
const apiurl = process.env.NEXT_PUBLIC_APIURL;
      ...
      setInvoiceData(prev => ({
        ...
        products: cart.map((product: any, idx: number) => {
          return {
            ...
            "Product Description": product.long_description || product.short_description || "",
            "Qty": product.cartQty || 1,
            "Rate (₹)": product.price || 0,
            "GST %": product.gst || 0,
            "GST Amt (₹)": 0,
            "Transport Cost (₹)": product.transportCost || 0,
            "Total (₹)": 0
          };
        })
      }));
  ...
  const calculatedProducts = invoiceData.products.map((row) => {
    ...
    const price = Number((row as any)["Rate (₹)"] || 0) > 0
      ? Number((row as any)["Rate (₹)"])
      : Number((row as any)["Rate (₹)"] || 0);
  ...
  const renderPreview = () => (
    <div className="overflow-x-auto w-full flex justify-center mt-8">
      <div className="bg-white p-6 rounded-lg shadow-lg border-2 border-gray-300 max-w-[800px] w-full min-w-[600px]">
        <h2 className="text-xl font-bold text-center mb-2">INVOICE B2B</h2>
        ...
        <div className="mb-2">
          <b>Transport Mode:</b> {invoiceData.transportMode || '-'}<br />
          <b>E-Way Bill No.:</b> {invoiceData.ewayBillNo || '-'}<br />
          <b>Vehicle Number:</b> {invoiceData.vehicleNumber || '-'}<br />
          <b>Remarks/Notes:</b> {invoiceData.remarks || '-'}<br />
          <b>Authorized Signature:</b> {invoiceData.authorizedSignature || '-'}
        </div>
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input className="w-full border rounded px-2 py-1 mb-2" placeholder="Transport Mode (e.g. Road/Air)" value={invoiceData.transportMode || ''} onChange={e => handleRootChange("transportMode", e.target.value)} />
            <input className="w-full border rounded px-2 py-1 mb-2" placeholder="E-Way Bill No." value={invoiceData.ewayBillNo || ''} onChange={e => handleRootChange("ewayBillNo", e.target.value)} />
            <input className="w-full border rounded px-2 py-1 mb-2" placeholder="Vehicle Number" value={invoiceData.vehicleNumber || ''} onChange={e => handleRootChange("vehicleNumber", e.target.value)} />
            <input className="w-full border rounded px-2 py-1 mb-2" placeholder="Remarks/Notes" value={invoiceData.remarks || ''} onChange={e => handleRootChange("remarks", e.target.value)} />
            <input className="w-full border rounded px-2 py-1 mb-2" placeholder="Authorized Signature (Name & Designation)" value={invoiceData.authorizedSignature || ''} onChange={e => handleRootChange("authorizedSignature", e.target.value)} />
            {/* Extra Fields */}
